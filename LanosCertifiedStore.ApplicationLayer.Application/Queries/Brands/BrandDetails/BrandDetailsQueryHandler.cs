...
{
    public async Task<Result<BrandDto>> Handle(BrandDetailsQuery request, CancellationToken cancellationToken)
    {
        var brand = await unitOfWork.RetrieveRepository<VehicleBrand>().GetEntityByIdAsync(request.Id);
